...
function showModal(section){
  closeModal(); const ov=document.createElement('div'); ov.className='modal-overlay'; ov.id='addModal';
  const db=section==='food'?FOOD_DB:(section==='supp'?SUPP_DB:(MED_DB||[]));
  window._curDB=db; window._curSec=section;
  let mealOpt='';
  if(section==='food') meal0pt='<div class="form-group"><label>餐次</label><select id="mealType"><option>早餐</option><option selected>午餐</option><option>晚餐</option><option>加餐</option></select></div>';
...
function filterDb(q){
  ...
  const db=window._curDB||[]; const r=(q||'').trim().toLowerCase();
...
function doConfirm(section){
  ...
  arr.push({id:p.id,name:p.name,mealType:section==='food'?document.getElementById('mealType').value:'',qty:q,unitLabel:p.unit||'',calories:(p.calories||0),calcium:(p.calcium||0),vitB1:(p.vitB1||0),vitB2:(p.vitB2||0),vitB6:(p.vitB6||0),vitC:(p.vitC||0)});
  saveData(appData); closeModal(); render();
}
function closeModal(){ const m=document.getElementById('addModal'); if(m)m.remove(); window._picked=null; }

// === Part 7: HTML 工具函数 ===
function emptyDiv(t){ const d=document.createElement('div'); d.className='empty-tip'; d.textContent=t; return d; }
...
function makeNutCard(t){
  ...
  const nutri=[{k:'calories',L:'热量',U:'kcal',C:'#f97316'},{k:'calcium',L:'钙',U:'mg',C:'#3b82f6'},{k:'vitB1',L:'维生素B1',U:'mg',C:'#a855f7'},{k:'vitB2',L:'维生素B2',U:'mg',C:'#ec4899'},{k:'vitB6',L:'维生素B6',U:'mg',C:'#14b8a6'},{k:'vitC',L:'维生素C',U:'mg',C:'#f59e0b'}];
  c.innerHTML='<div class="card-title">今日营养摄入</div>'+nutri.map(n=>{
    const v=t[n.k]||0; const d=DRI[n.k]; const pct=Math.min(100,Math.round(v/d*100));
    const cls=pct>=80?'ok':pct>=50?'low':'deficient';
    const disp=n.k==='calories'?Math.round(v):v.toFixed(1);
    return '<div class="nutrient-row"><div class="nutrient-label"><span>'+n.I+' '+n.L+'</span><span class="status-badge '+cls+'">'+(pct>=80?'达标':pct>=50?'偏低':'不足')+'</span></div><div class="progress-wrap"><div class="progress-bar" style="width:'+pct+'%;background:'+n.C+'"></div></div><div class="nutrient-val">'+disp+' / '+d+' '+n.U+' ('+pct+'%)</div></div>';
  }).join('');
  return c;
}
function delRec(date,sec,idx){ if(!appData[date]||!appData[date][sec])return; appData[date][sec].splice(idx,1); saveData(appData); render(); }
...
function renderHistory(p){
  ...
  dates.forEach(date=>{
    ...
    card.innerHTML=`<div class="history-date">${fmtDate(date)}${date===todayKey()?'（今天）':''}</div>
      <div class="history-summary"><span class="hist-tag cal">🔥 ${Math.round(tot.calories)} kcal</span>
        <span class="hist-tag ca">🦷 钙 ${Math.round(tot.calcium)}mg</span>
        <span class="hist-tag vc">🍊 VC ${Math.round(tot.vitC)}mg</span>
        <span class="hist-tag b">B族 ${((tot.vitB1+tot.vitB2+tot.vitB6)).toFixed(1)}mg</span></div>
...
function renderTrends(p){
  ...
  const nutrients=[{k:'calories',L:'热量',U:'kcal',C:'#f97316'},{k:'calcium',L:'钙',U:'mg',C:'#3b82f6'},{k:'vitC',L:'维生素C',U:'mg',C:'#f59e0b'},{k:'vitB1',L:'维生素B1',U:'mg',C:'#a855f7'},{k:'vitB2',L:'维生素B2',U:'mg',C:'#ec4899'},{k:'vitB6',L:'维生素B6',U:'mg',C:'#14b8a6'}];
  nutrients.forEach(n=>{
    const vals=last7.map(dt=>{ const day=appData[dt]; if(!day)return 0; return calcTotals([...(day.food||[]),...(day.supp||[]))[n.k]||0; });
    const mx=Math.max(...vals,DRI[n.k]);
    const chartCard=document.createElement('div'); chartCard.className='chart-card';
    chartCard.innerHTML=`<div class="chart-title">${n.L}（${n.U}）</div>`;
    ...
    last7.forEach((dt,i)=>{
      ...
      col.innerHTML=`<div class="bar-val">${(n.k==='calories'?Math.round(v):v.toFixed(1))}</div><div class="bar-outer"><div class="bar-inner" style="height:${pct}%;background:${n.C}"></div><div class="dri-line" style="bottom:${dp}%"></div></div><div class="bar-date${dt===todayKey()?' today':''}">${dt===todayKey()?'今天':dt.slice(5)}</div>`;
    ...
    chartCard.appendChild(note); p.appendChild(chartCard);
  });
  p.appendChild(makeSuggestionCard(last7));
}
function makeSuggestionCard(last7){
  const card=document.createElement('div'); card.className='suggest-card';
  card.innerHTML='<div class="chart-title">💊 营养补充建议</div>';
  const avg={}; ['calcium','vitB1','vitB2','vitB6','vitC'].forEach(k=>{
    const vals=last7.map(dt=>{ const day=appData[dt]; if(!day)return 0; return calcTotals([...(day.food||[]),...(day.supp||[]))[k]||0; }).filter(v=>v>0);
  ...
  if(avg.vitB1<DRI.vitB1*0.8||avg.vitB2<DRI.vitB2*0.8||avg.vitB6<DRI.vitB6*0.8) sug.push({i:'💊',t:'维生素B族摄入不足，建议补充复合维生素B片'});
  if(!sug.length) card.innerHTML+='<div class="suggest-ok">🎉 近7天营养摄入均衡，继续保持！</div>';
  else sug.forEach(s=>{ card.innerHTML+=`<div class="suggest-item">${s.i} ${s.t}</div>`; });
  return card;
}

// === Part 11: PWA + SW ===
if('serviceWorker' in navigator){ window.addEventListener('load',()=>{ navigator.serviceWorker.register('./sw.js').catch()=>{}); }); }
window.addEventListener('beforeinstallprompt',e=>{ e.preventDefault(); window._dpt=e;
  if(!isPWAInstalled()){ const g=document.getElementById('installGuide'); if(g)g.style.display='block'; }
});
['installBtn','installBtn2'].forEach(id=>{
  const btn=document.getElementById(id); if(!btn)return;
  btn.addEventListener('click',async()=>{
    if(!window._dpt){ alert('请在浏览器菜单中选择「添加到主屏幕」'); return; }
    window._dpt.prompt(); const{r}=await window._dpt.userChoice;
    if(r==='accepted') hideInstall();
    window._dpt=null;
  });
});
